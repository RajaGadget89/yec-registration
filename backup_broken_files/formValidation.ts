...
const validateBusinessTypeOther = (
  value: string,
  formData?: Record<string, unknown>,
): ValidationResult => {
...
const validateExternalHotelName = (
  value: string,
  formData?: Record<string, unknown>,
): ValidationResult => {
  ...
  formSchema.forEach((field) => {
    ...
    if (field.extraField) {
      const extraFieldValidation = validateField(
        {
          ...field.extraField,
          required: shouldFieldBeRequired({ id: field.extraField.id }, formData),
        },
    ...
    if (field.roommatePhoneField && shouldFieldBeRequired({ id: field.roommatePhoneField.id }, formData)) {
      ...
      if (!roommatePhoneValidation.isValid) {
        errors[field.roommatePhoneField.id] =
          roommatePhoneValidation.message || "";
        isValid = false;
      }
    }
  });
...
export const shouldShowExtraField = (
  field: FormField,
  formData: Record<string, unknown>,
): boolean => {
  switch (field.id) {
    ...
    case "roomType":
      return (
        formData.roomType === "double" && formData.hotelChoice === "in-quota"
      );
    default:
      return false;
  }
...
export const shouldFieldBeRequired = (
  field: FormField,
  formData?: Record<string, unknown>,
): boolean => {
...
export const validateRegistrationData = (data: any) => {
  ...
  if (
    data.phone &&
    !/^0[0-9]{9}$/.test(data.phone) &&
    !/^\+66[0-9]{8}$/.test(data.phone)
  ) {
  ...
  if (data.lineId && !/^[a-zA-Z0-9._-]+$/.test(data.lineId)) {
    errors.push(
      "Line ID can only contain letters, numbers, dots, underscores, and hyphens",
    );
  }
  ...
  if (
    ...
    (!data.roommateInfo || !data.roommatePhone)
  ) {
