-- Migration: Admin Team Management (Phase 1) - FIXED VERSION
-- Date: 2025-01-27
-- Description: Add admin invitation system and extend admin_users table using idempotent approach
...
-- 2. Create admin_invitations table (idempotent)
...
-- 3. Create indexes for better performance (idempotent)
...
-- 4. Enable RLS on admin_invitations table (idempotent)
ALTER TABLE admin_invitations ENABLE ROW LEVEL SECURITY;

-- 5. Create RLS policies using idempotent approach
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public'
          ...
          AND policyname = 'Super admins can manage admin invitations'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public'
          AND tablename = 'admin_invitations'
          AND policyname = 'Service role can manage admin invitations'
    ) THEN
...
-- 7. Create trigger for updated_at (idempotent)
