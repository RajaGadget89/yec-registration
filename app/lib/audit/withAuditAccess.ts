  ...
  return async (request: NextRequest, ...args: T): Promise<NextResponse> => {
    ...
    try {
      if (config.skipLogging) {
        // ★ สำคัญ: return ค่า handler ตรง ๆ
      ...
      // ★ สำคัญ: รอผลและ return “response” ที่ได้จาก handler
      ...
      safeLogAccess({
        ...
        meta: { wrapper_version: "1.0.0", timestamp: new Date().toISOString() },
      });

      response.headers.set("x-request-id", requestId);
      return response; // ★ return เสมอ
    } catch (error) {
      statusCode = 500;
