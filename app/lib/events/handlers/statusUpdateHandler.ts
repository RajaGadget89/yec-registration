    ...
    try {
      switch (event.type) {
        case 'registration.submitted':
          ...
          console.log(`Registration ${event.payload.registration.registration_id} submitted - status handled by trigger`);
          break;
        ...
        case 'admin.approved':
          ...
          if (approveError) {
            throw new Error(`Failed to update registration status to approved: ${approveError.message}`);
          }

          console.log(`Updated registration ${event.payload.registration.registration_id} status to approved`);
          break;
        ...
        case 'admin.rejected':
          ...
          if (rejectError) {
            throw new Error(`Failed to update registration status to rejected: ${rejectError.message}`);
          }

          console.log(`Updated registration ${event.payload.registration.registration_id} status to rejected`);
          break;
        ...
        case 'document.reuploaded':
          ...
          if (reuploadError) {
            throw new Error(`Failed to update registration status after re-upload: ${reuploadError.message}`);
          }

          console.log(`Updated registration ${event.payload.registration.registration_id} status to waiting_for_review after re-upload`);
          break;
        ...
        case 'auto_reject.sweep_completed':
          ...
          console.log(`Auto-reject sweep completed, ${autoRejectPayload.rejected_registrations?.length || 0} registrations rejected`);
          break;

        default:
          console.log(`StatusUpdateHandler: No action needed for event type ${event.type}`);
      }
    } catch (error) {
