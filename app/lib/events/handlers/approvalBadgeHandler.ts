...
export class ApprovalBadgeHandler implements EventHandler<RegistrationEvent> {
  async handle(event: RegistrationEvent): Promise<void> {
    console.log(`🔍 ApprovalBadgeHandler.handle called with event type: ${event.type}`);
    try {
      switch (event.type) {
        case "admin.approved":
  ...
  private async handleAdminApproved(event: RegistrationEvent) {
    console.log(`🔍 ApprovalBadgeHandler.handleAdminApproved called with event:`, {
      type: event.type,
    ...
    if (registration.status === 'approved') {
      console.log(`🏆 Admin approved - generating approval badge for: ${registration.registration_id}`);
      try {
        // Always use fresh data from database, not event payload data
        const badgeUrl = await approvalBadgeService.generateApprovalBadge(registration.registration_id);
        console.log(`✅ Approval badge generated successfully: ${badgeUrl}`);
      } catch (error) {
        console.error(`❌ Approval badge generation failed for ${registration.registration_id}:`, error);
      }
    } else {
      console.log(`⚠️ Registration not approved, status: ${registration.status}, skipping badge generation`);
    }
  }
  ...
  private async handleUserResubmitted(event: RegistrationEvent) {
    ...
    if (registration.status === 'approved') {
      console.log(`🔄 User resubmitted - regenerating badge for: ${registration.registration_id}`);
