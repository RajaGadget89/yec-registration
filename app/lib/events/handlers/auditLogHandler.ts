...
export class AuditLogHandler implements EventHandler<RegistrationEvent> {
  async handle(event: RegistrationEvent): Promise<void> {
    ...
    try {
      switch (event.type) {
        ...
        default:
          console.warn(
            `Unhandled event type in AuditLogHandler: ${event.type}`,
          );
      }
    } catch (error) {
  ...
  private async logAdminAction(
    ...
    if (
      event.type !== "admin.request_update" &&
    ...
    if (error) {
      ...
      if (
        error.message.includes('relation "admin_audit_logs" does not exist')
      ) {
        console.log(
          "Admin audit logs table does not exist, skipping audit logging",
        );
        return;
      }
      throw error;
    ...
    console.log(
      `Audit log entry created for ${action} action on registration ${registration.registration_id}`,
    );
  }
