...
export class AuditDomainHandler implements EventHandler<RegistrationEvent> {
  async handle(event: RegistrationEvent): Promise<void> {
    try {
      ...
      if (process.env.NODE_ENV === 'test' || process.env.PLAYWRIGHT_TEST) {
        console.debug(`[AUDIT] Processing event ${event.type} with correlation_id ${correlationId}`);
      }
      ...
      switch (event.type) {
        ...
        default:
          console.warn(`Unhandled event type in AuditDomainHandler: ${(event as any).type}`);
      }
    } catch (error) {
  ...
  private async handleRegistrationSubmitted(event: RegistrationEvent, correlationId: string): Promise<void> {
  ...
  private async handleRegistrationBatchUpserted(event: RegistrationEvent, correlationId: string): Promise<void> {
  ...
  private async handleAdminRequestUpdate(event: RegistrationEvent, correlationId: string): Promise<void> {
  ...
  private async handleAdminApproved(event: RegistrationEvent, correlationId: string): Promise<void> {
    const { registration, reason } = event.payload as any;
  ...
  private async handleAdminRejected(event: RegistrationEvent, correlationId: string): Promise<void> {
    const { registration, reason } = event.payload as any;
  ...
  private async handleDocumentReuploaded(event: RegistrationEvent, correlationId: string): Promise<void> {
  ...
  private async handleStatusChanged(event: RegistrationEvent, correlationId: string): Promise<void> {
    const { registration, beforeStatus, afterStatus, reason, actorRole } = event.payload as any;
  ...
  private async handleLoginSubmitted(event: RegistrationEvent, correlationId: string): Promise<void> {
  ...
  private async handleLoginSucceeded(event: RegistrationEvent, correlationId: string): Promise<void> {
