...
export async function uploadFileToSupabaseClient(
  ...
  try {
    ...
    if (file.size > maxFileSize) {
      throw new Error(`File size (${file.size} bytes) exceeds maximum allowed size (${maxFileSize} bytes)`);
    }
    
    // Check MIME type
    if (!allowedMimeTypes.includes(file.type)) {
      throw new Error(`File type (${file.type}) not allowed. Allowed types: ${allowedMimeTypes.join(', ')}`);
    }
