...
export class CheckinProcessor {
  ...
  async getAttendanceStats() {
    try {
      ...
      const { data: attendanceByEvent } = await this.supabase
        .from('user_checkins')
        .select(`
          checkin_events!inner(
      ...
      return {
        ...
        stats: {
          ...
          attendance_rate: totalUsers ? ((checkedInUsers || 0) / totalUsers * 100).toFixed(2) : 0,
          by_event_type: Object.entries(eventTypeStats).map(([event_type, count]) => ({
            event_type,
            count
          }))
        }
