...
export class AttendanceStats {
  ...
  async getAttendanceByEventType() {
    try {
      ...
      // Group by event type
      const eventTypeStats: Record<string, { count: number; events: string[] }> = {};
      if (checkins) {
        checkins.forEach((checkin: any) => {
          const eventType = checkin.checkin_events?.event_types?.name;
          ...
          if (eventType) {
            ...
            eventTypeStats[eventType].count++;
            if (eventName && !eventTypeStats[eventType].events.includes(eventName)) {
  ...
  async getUserAttendanceSummary(registrationId: string) {
    try {
      ...
      return {
        ...
        by_event_type: Object.entries(eventTypeCounts).map(([event_type, count]) => ({
          event_type,
          count
        }))
      };
  ...
  async getFirstSightBadgeDistribution() {
    try {
      ...
      try {
        ...
        console.log('business_rule_category not available, falling back to event type name for recipients');
      ...
      return {
        ...
        completion_rate: totalEligible ? (((badgesIssued || 0) / totalEligible) * 100).toFixed(2) : 0,
        badge_recipients: badgeRecipients?.map((recipient: any) => ({
        ...
        pending_users: pendingUsers?.map((user: any) => ({
