...
export async function validateInvitationToken(
  token: string,
): Promise<InvitationTokenValidation> {
  const supabase = getSupabaseServiceClient();
...
export function mapInvitationStatus(
  ...
  newStatus: "accepted" | "expired" | "revoked",
): string {
  if (currentStatus !== "pending") {
    throw new Error(
      `Cannot change status from ${currentStatus} to ${newStatus}. Only pending invitations can be modified.`,
    );
  }
