...
export class PricingCalculator {
  ...
  static async calculatePrice(
    ...
    const price = isEarlyBird
      ? selectedPackage.early_bird_amount
      : selectedPackage.regular_amount;
   ...
   */
  static async isEarlyBirdAvailable(
    currentTimeUtc: Date = new Date(),
  ): Promise<boolean> {
    const eventSettings = await this.getEventSettings();
    const earlyBirdDeadline = new Date(eventSettings.early_bird_deadline_utc);
  ...
  static convertToUtc(bangkokTime: string): Date {
    ...
    return new Date(
      bangkokDate.getTime() - bangkokDate.getTimezoneOffset() * 60000,
    );
  }
}
