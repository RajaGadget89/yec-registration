...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    const handleAuthCallback = async () => {
      try {
        console.log('[callback] starting auth flow at:', new Date().toISOString());
        ...
        if (!accessToken || !refreshToken) {
          ...
          setStatus('error');
          setErrorMessage('No access token or refresh token found in URL. Please request a new magic link.');
          return;
        }
        ...
        if (accessToken.split('.').length !== 3) {
          ...
          setStatus('error');
          setErrorMessage('Invalid magic link format. Please request a new one.');
          return;
        }
        ...
        if (response.status === 303 || response.redirected) {
          console.log('[callback] 303 response or redirect detected, checking headers...');
          ...
          if (location) {
            ...
            console.log('[callback] browser followed redirect, checking current location...');
            
            // Check if we're already on the admin page
            if (window.location.pathname === '/admin') {
              console.log('[callback] already on admin page, authentication successful!');
              setStatus('success');
              setSuccessMessage('Authentication successful! You are now on the admin dashboard.');
              
              // Clean the URL
              history.replaceState({}, '', '/admin');
            } else {
              // Redirect happened but we're not on admin page
              console.log('[callback] redirect followed but not on admin page, redirecting manually...');
              setStatus('success');
              setSuccessMessage('Authentication successful! Redirecting to admin dashboard...');
            ...
            console.error('[callback] redirect response but no Location header');
            setStatus('error');
            setErrorMessage('Authentication succeeded but redirect failed. Please try again.');
          }
          return; // Exit early for 303 responses
        }
        
        // Handle error responses (only if not 303 and not redirected)
        console.log('[callback] handling error response, status:', response.status);
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-white/20">
          {status === 'loading' && (
            <div className="text-center">
              ...
              <h1 className="text-xl font-bold text-gray-900 mb-2">Processing Authentication</h1>
