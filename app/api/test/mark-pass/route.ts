...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Failed to mark dimension as passed' }, { status: 500 });
    }
    ...
    if (allPassed) {
      ...
      const { data: approveResult, error: approveError } = await supabase
        .rpc('fn_try_approve', { reg_id: registrationId });
    ...
    try {
      await EventService.emitAdminMarkPass(registration, 'test-admin@example.com', dimension);
