...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!bucketExists) {
      ...
      if (createError) {
        throw new Error(
          `Failed to create yec-assets bucket: ${createError.message}`,
        );
      }

      console.log(
        "[SETUP-EMAIL-ASSETS] yec-assets bucket created successfully",
      );
    } else {
    ...
    console.log(
      "[SETUP-EMAIL-ASSETS] Logo file read successfully, size:",
      logoBuffer.length,
    );
