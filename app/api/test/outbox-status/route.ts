...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: stats, error: statsError } = await supabase.rpc('fn_get_outbox_stats');
    ...
    const { data: pendingEmails, error: pendingError } = await supabase.rpc('fn_get_pending_emails', { limit: 10 });
