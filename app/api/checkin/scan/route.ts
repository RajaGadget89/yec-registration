...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { registration_id, checkin_event_id, location, notes } =
      await req.json();
    ...
    if (!registration_id || !checkin_event_id) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: registration_id and checkin_event_id",
        },
        { status: 400 },
      );
    }
    ...
    const eventTypeName = event.event_types?.[0]?.name;
    ...
    if (event.event_types?.[0]?.business_rule_category === "ONE_TIME_ONLY") {
      ...
      const { data: previousCheckins, error: checkinsError } = await supabase
        ...
        .eq("registration_id", registration_id)
        .eq("event_type_id", event.event_types?.[0]?.id); // Direct relationship
      ...
      if (!checkinsError && previousCheckins && previousCheckins.length > 0) {
        ...
        return NextResponse.json(
          {
            error:
              "User has already completed this type of check-in. Each user can only complete this type once.",
            checkin_time: previousCheckins[0].checkin_time,
            business_rule:
              "ONE_TIME_ONLY events are restricted to one check-in per user",
          },
          { status: 409 },
        );
      }
    ...
    const { data: checkin, error } = await supabase
      ...
      .insert({
        registration_id,
        checkin_event_id,
        event_type_id: event.event_types?.[0]?.id, // Add event_type_id for improved validation
      ...
      return NextResponse.json(
        {
          ...
          event_type_id: event.event_types?.[0]?.id,
        },
        { status: 500 },
      );
    }
    ...
    await logEvent({
      ...
      actor_id: user.email,
      actor_role: user.role === "super_admin" ? "admin" : user.role,
      result: "success",
