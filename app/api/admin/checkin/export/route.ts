...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log('[Export API] User authenticated:', user.email, 'Role:', user.role);
    ...
    console.log('[Export API] Parameters:', { format, search, eventType, dateFrom, dateTo, province, sortBy, sortOrder });
    ...
    if (!checkins || checkins.length === 0) {
      ...
      const headers = ['Check-in Time', 'User Name', 'Email', 'Event Name', 'Location', 'Notes'];
    ...
    console.log('[Export API] Data already filtered by database query, processing', checkins.length, 'records');

    // Format the data for export
    console.log('[Export API] Formatting export data for', checkins.length, 'records');
    ...
    try {
      exportData = checkins.map((checkin: any) => {
        ...
        return {
          'Check-in Time': checkin.checkin_time ? new Date(checkin.checkin_time).toLocaleString('en-GB', {
            day: '2-digit',
            month: '2-digit', 
            year: 'numeric',
            hour: '2-digit',
          ...
          'User Name': reg ? `${reg.first_name || ''} ${reg.last_name || ''}`.trim() : '',
          'Email': reg?.email || '',
      ...
      console.log('[Export API] Successfully formatted', exportData.length, 'export records');
    } catch (formatError) {
      console.error('[Export API] Error formatting export data:', formatError);
      return NextResponse.json({ error: "Failed to format export data", details: formatError.message }, { status: 500 });
    }
    ...
    if (format === 'csv') {
      ...
      try {
        ...
        if (!exportData || exportData.length === 0) {
          console.log('[Export API] No data to export, returning empty CSV with headers');
          const headers = ['Check-in Time', 'User Name', 'Email', 'Event Name', 'Location', 'Notes'];
          const csvContent = headers.join(',') + '\n';
        ...
        return NextResponse.json({ error: "Failed to generate CSV", details: csvError.message }, { status: 500 });
      }
    } else {
      // Return JSON
