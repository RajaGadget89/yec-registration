...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingBranding) {
      return NextResponse.json({ error: 'Active branding configuration already exists. Use PUT to update.' }, { status: 400 });
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!currentBranding) {
      return NextResponse.json({ error: 'No active branding configuration found. Use POST to create.' }, { status: 404 });
    }
