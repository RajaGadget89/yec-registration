  ...
  try {
    ...
    const analysis = {
      title: {
        ...
        score: validatedData.title.length >= 30 && validatedData.title.length <= 60 ? 100 : 
               validatedData.title.length < 30 ? (validatedData.title.length / 30) * 100 : 
      ...
      metaDescription: {
        ...
        score: validatedData.meta_description ? 
               (validatedData.meta_description.length >= 120 && validatedData.meta_description.length <= 160 ? 100 :
                validatedData.meta_description.length < 120 ? (validatedData.meta_description.length / 120) * 100 :
                Math.max(0, 100 - ((validatedData.meta_description.length - 160) * 2))) : 0,
        issues: []
      },
      content: {
    ...
    if (analysis.title.length < 30) {
      analysis.title.issues.push('Title is too short (recommended: 30-60 characters)');
    }
    if (analysis.title.length > 60) {
      analysis.title.issues.push('Title is too long (recommended: 30-60 characters)');
    }
    ...
    if (!validatedData.meta_description) {
      ...
      analysis.metaDescription.issues.push('Meta description is too short (recommended: 120-160 characters)');
    } else if (analysis.metaDescription.length > 160) {
      analysis.metaDescription.issues.push('Meta description is too long (recommended: 120-160 characters)');
    }

    if (analysis.content.wordCount < 300) {
      analysis.content.issues.push('Content is too short (recommended: 300+ words)');
    }
    ...
    analysis.overall.score = Math.round(
      (analysis.title.score + analysis.metaDescription.score + analysis.content.readability) / 3
    );

    // Generate recommendations
    if (analysis.title.score < 80) {
      analysis.overall.recommendations.push('Optimize title length and keywords');
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to update SEO optimizations' }, { status: 500 });
    }
...
function calculateReadabilityScore(content: string): number {
  ...
  const syllables = content.toLowerCase().replace(/[^a-z]/g, '').split('').length;
