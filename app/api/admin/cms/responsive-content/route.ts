...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to fetch responsive content' }, { status: 500 });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingContent) {
      return NextResponse.json({ error: 'Responsive content already exists for this section and device type' }, { status: 400 });
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to create responsive content' }, { status: 500 });
    }
