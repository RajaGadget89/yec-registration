...
export const POST = withAuditLogging(async (request: NextRequest) => {
  try {
    ...
    const { data: registration, error: fetchError } = await supabase
      .from("registrations")
      .select("id, email, status, payment_review_status")
    ...
    // Update payment review status to approved
    ...
    if (updateError) {
      console.error("Error updating payment review status:", updateError);
      return NextResponse.json(
        { error: "Failed to update payment review status" },
        { status: 500 },
      );
    }
    ...
    await safeLogAccess({
      ...
      method: "POST",
      resource: "/api/admin/registrations/approve-payment",
      result: "success",
