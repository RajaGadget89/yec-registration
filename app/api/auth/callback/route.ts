...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[api/callback] Supabase URL:', process.env.NEXT_PUBLIC_SUPABASE_URL);
    console.log('[api/callback] Service role key exists:', !!process.env.SUPABASE_SERVICE_ROLE_KEY);
    ...
    console.log('[api/callback] admin emails from env:', process.env.ADMIN_EMAILS);
    ...
    const maskToken = (token: string) => 
      token ? `${token.substring(0, 4)}...${token.substring(token.length - 4)}` : 'null';
