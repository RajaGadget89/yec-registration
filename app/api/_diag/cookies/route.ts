...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = {
      ...
      maskedTokens: {
        accessToken: accessToken ? `${accessToken.substring(0, 4)}...${accessToken.substring(accessToken.length - 4)}` : null,
        refreshToken: refreshToken ? `${refreshToken.substring(0, 4)}...${refreshToken.substring(refreshToken.length - 4)}` : null,
      }
    };

    console.log('[auth-debug] _diag/cookies: checking cookies');
    console.log('[auth-debug] _diag/cookies: auth cookies present:', Object.values(result.authCookies).filter(v => v === 'present').length, '/ 3');
    console.log('[auth-debug] _diag/cookies: total cookies:', result.totalCookies);
