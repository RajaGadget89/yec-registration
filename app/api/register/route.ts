...
import { validateRegistrationData, mapFrontendToDatabase } from '../../components/RegistrationForm/formValidation';
  ...
  try {
    ...
    if (!isRegistrationOpen) {
      return NextResponse.json({
        ...
        message: 'Registration deadline has passed. Registration is no longer accepting new submissions.'
      }, { status: 400 });
    }
    ...
    if (body.selectedPackage) {
      try {
        const pricingResult = await PricingCalculator.calculatePrice(body.selectedPackage);
        ...
        return NextResponse.json({
          error: 'Pricing error',
          message: 'Failed to calculate registration price. Please try again.'
        }, { status: 500 });
      }
    }
    ...
    const { data: registration, error } = await supabase.from('registrations').insert([insertPayload]).select().single();
    ...
    return NextResponse.json({
      success: true,
      message: 'Registration submitted successfully and is waiting for admin review',
      registration_id: registration.registration_id,
      price_applied: priceApplied,
      currency: currency,
      is_early_bird: priceApplied ? await PricingCalculator.isEarlyBirdAvailable() : null,
    });
