...
export async function GET(request: NextRequest) {
  ...
  const requestId = `mcp_admin_regs_${start}_${Math.random().toString(36).slice(2, 8)}`;

  const auth = await validateMCPApiKey(request.headers);
  if (!auth.ok || auth.type !== "admin") {
    const res = NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    await auditMCPAccess({
      endpoint: "/api/mcp/admin/registrations",
      method: "GET",
  ...
  const rl = rateLimit(`mcp:admin:registrations:${auth.type}`);
  if (!rl.allowed) {
  ...
  try {
    ...
    const { data: regs, error, count } = await query;
