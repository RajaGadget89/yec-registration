...
/**
 * Auto-redirect from /admin/login when a valid session already exists.
 * Also attempts a server cookie bridge if the client has a session
 * but the server (/api/admin/me) still returns 401.
 */
function AutoRedirectOnSession() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // Create client using existing project helper
        ...
        console.log("[login-auto] found client session, checking server");

        // 2) Confirm server sees the session
        const me = await fetch("/api/admin/me", { credentials: "include" });
        if (me.status === 200) {
          console.log("[login-auto] server session confirmed, redirecting");
        ...
        console.log(
          "[login-auto] server session not found, attempting cookie bridge",
        );

        // 3) Optional server cookie bridge if available
        // If your app has POST /api/auth/session (cookie bridge), call it.
        // This covers cases where client has tokens but server cookie isn't set yet.
        ...
        if (at && rt) {
          try {
            ...
            const me2 = await fetch("/api/admin/me", {
              credentials: "include",
            });
            if (me2.status === 200) {
              console.log("[login-auto] cookie bridge successful, redirecting");
              // Add small delay to ensure cookies are properly set
              setTimeout(() => {
                router.replace(next);
              }, 100);
              ...
              console.log(
                "[login-auto] cookie bridge failed, staying on login",
              );
            }
          } catch (e) {
            // If bridge route doesn't exist or fails, silently ignore
            // and keep the user on the login page.
            console.warn("[login-auto] cookie bridge failed:", e);
          }
        }
      } catch (e) {
        console.warn("[login-auto] check failed:", e);
      }
    })();
...
export default function AdminLoginPage() {
  ...
  const handleMagicLink = async () => {
    ...
    try {
      // Use server-side magic link endpoint to bypass rate limiting
      ...
      if (!response.ok) {
        console.error("[admin-login] Magic link error:", data);
