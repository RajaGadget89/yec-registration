...
export default function UserCheckinsPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/checkin/users/${searchTerm.trim()}`);
      ...
      <div className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">User Check-in History</h1>
              <p className="text-sm text-gray-600">Search and view user check-in records</p>
            </div>
            <button
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
              ...
              <div className="px-6 py-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-900">{searchResults.user_info.full_name}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-gray-500">Email:</span>
                    <p className="text-sm text-gray-900">{searchResults.user_info.email}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-gray-500">Phone:</span>
                    <p className="text-sm text-gray-900">{searchResults.user_info.phone}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-gray-500">Province:</span>
                    <p className="text-sm text-gray-900">{searchResults.user_info.yec_province}</p>
                  </div>
                  <div>
                    <span className="text-sm font-medium text-gray-500">Registration ID:</span>
                    <p className="text-sm text-gray-900">{searchResults.registration_id}</p>
                  </div>
                  <div>
                          ...
                          <div className="flex-1">
                            ...
                            {checkin.metadata && (
                              <div className="mt-1 text-xs text-gray-500">
                                <span className="font-medium">Checked by:</span> {checkin.admin_users.email}
                              </div>
                            )}
                          </div>
        ...
        {!searchResults && (
          <div className="bg-white shadow rounded-lg p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">How to Search</h3>
            <div className="space-y-3 text-sm text-gray-600">
              <p>1. Enter the user's registration ID in the search field above</p>
