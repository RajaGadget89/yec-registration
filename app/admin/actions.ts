...
export async function getRegistrations(
  ...
  const statusCounts = {
    ...
    pending: statusCountsData?.filter(r => r.status === 'pending').length || 0,
    waiting_for_review: statusCountsData?.filter(r => r.status === 'waiting_for_review').length || 0,
...
export async function getProvinces(): Promise<string[]> {
  ...
  const provinces = Array.from(new Set(data?.map(r => r.yec_province).filter(Boolean) || []));
