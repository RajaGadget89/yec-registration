...
export default function AuditTable({ data, type }: AuditTableProps) {
  ...
  const getStatusColor = (result: string) => {
    if (result.startsWith('2')) return 'text-green-600 bg-green-50 dark:bg-green-900/20';
    if (result.startsWith('4')) return 'text-yellow-600 bg-yellow-50 dark:bg-yellow-900/20';
    if (result.startsWith('5')) return 'text-red-600 bg-red-50 dark:bg-red-900/20';
          ...
          {data.map((row) => (
            <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(row.result)}`}>
                  {row.result}
                </span>
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                <div className="flex items-center space-x-2">
                  <div className="max-w-xs truncate font-mono text-xs" title={row.request_id || row.correlation_id}>
                    {row.request_id || row.correlation_id}
                  </div>
                  <CopyButton text={row.request_id || row.correlation_id || ''} />
                </div>
              </td>
              {type === 'access' && (
                <>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                    <div className="max-w-xs truncate font-mono text-xs" title={row.src_ip || ''}>
                      {row.src_ip || '-'}
                    </div>
                  </td>
                </>
