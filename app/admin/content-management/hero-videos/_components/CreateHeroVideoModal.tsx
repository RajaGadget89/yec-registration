...
interface CreateHeroVideoModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function CreateHeroVideoModal({
  isOpen,
  onClose,
  onSuccess,
}: CreateHeroVideoModalProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.page_id) {
      newErrors.page_id = "Please select a page";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to create hero video");
      }
              ...
              >
                ...
                {pages.map((page) => (
                  <option key={page.id} value={page.id}>
                    {page.title} ({page.slug}) - {page.language.toUpperCase()}
                  </option>
                ))}
              </select>
            )}
            {errors.page_id && (
              <p className="mt-1 text-sm text-red-600">{errors.page_id}</p>
            )}
          </div>
