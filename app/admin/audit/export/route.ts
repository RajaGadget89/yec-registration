...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Export: Request params:', { type, request_id, action, resource, date_from, date_to });
    ...
    if (type === 'access') {
      ...
      headers = ['occurred_at_th', 'action', 'resource', 'result', 'request_id', 'latency_ms', 'src_ip', 'user_agent'];
      csvContent = headers.join(',') + '\n';
      
      // Add data rows
      for (const log of logs) {
        try {
          const thTime = toZonedTime(new Date(log.occurred_at_utc), 'Asia/Bangkok');
          ...
          console.error('Export: Error processing access log row:', rowError, log);
      ...
      headers = ['occurred_at_th', 'action', 'resource', 'result', 'correlation_id'];
      csvContent = headers.join(',') + '\n';
      ...
      for (const log of logs) {
        try {
          ...
          console.error('Export: Error processing event log row:', rowError, log);
          // Continue with other rows
        }
      }
