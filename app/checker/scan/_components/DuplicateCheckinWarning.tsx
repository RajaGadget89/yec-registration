...
export default function DuplicateCheckinWarning({
  ...
  const formatEventType = (type: string) => {
    ...
    return eventTypeMap[type] || type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  };
        ...
        <div className="p-4">
              ...
              <div className="flex-shrink-0">
                <div className="h-8 w-8 bg-yec-primary rounded-full flex items-center justify-center">
                  <span className="text-white text-sm font-medium">
                    {userInfo.fullName ? userInfo.fullName.split(' ').map(n => n[0]).join('').toUpperCase() : 'U'}
                  </span>
                </div>
              </div>
              <div className="ml-3">
                <p className="text-sm font-medium text-gray-900">{userInfo.fullName || 'Unknown User'}</p>
                <p className="text-xs text-gray-500">{userInfo.email || 'unknown@example.com'}</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-red-700">
                  <p className="mb-2">
                    This user has already checked in to this event type previously.
                  </p>
                  ...
                  <p className="mt-2 text-xs">
                    <strong>Business Rule:</strong> Each user can only check in once per event type.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mt-4 text-center">
            <p className="text-xs text-gray-500">
              Select a different event type or verify the user's check-in status
            </p>
          </div>
        </div>
      </div>
